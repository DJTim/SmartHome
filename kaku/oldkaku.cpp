#include <wiringPi.h>
#include <stdio.h>
#include <stdlib.h>
#include <getopt.h>
#include <unistd.h>
#include <ctype.h>
#include <iostream>
#include "RemoteTransmitter.cpp"

using namespace std;

int main(int argc, char **argv) 
{
	int pin_out = 15; // Pin out using wiringPi pin numbering scheme (15 = TxD / BCM GPIO 14, see https://projects.drogon.net/raspberry-pi/wiringpi/pins/)
    
    char address = 'A';
    int device = 0;
    int state = 0;
    
    
    if( argc != 4 ) { // not enough arguments
    	std::cout << "usage: " << argv[0] << " address device state" << std::endl;
    	std::cout << "example: " << argv[0] << " A 1 0" << std::endl;
	    return 0;
    } else {
    	address = argv[1][0];
	    device = atol(argv[2]);
	    state = atol(argv[3]);
    }
    
    // load wiringPi
	if(wiringPiSetup() == -1)
	{
		printf("WiringPi setup failed. Maybe you haven't installed it yet?");
		exit(1);
	}

    // setup pin and make it low
	pinMode(pin_out, OUTPUT);
	digitalWrite(pin_out, LOW);
	
	KaKuTransmitter transmitter(pin_out, 375, 4);
	
	if (state == 0){
		transmitter.sendSignal(address,device,false);
	}else{
		transmitter.sendSignal(address,device,true);
	}
}


